...
#if defined(NDEBUG)
#error "CC cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 1 * 60; // TODO CHANGE DEMO
...
const string strMessageMagic = "CC Signed Message:\n";
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int tier, int nMasternodeCount)
{
    ...
    } else if (nHeight > 2100000 && nHeight <= 3150000) { // 104.125/208.25/505.75
		switch(tier) {
        case 1:
    	    ret = 104.125 * COIN;
            break;
    ...
    } else if (nHeight > 3150000) { // 72.8875/145.775/354.025
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
 ...
 //           REJECT_INVALID, "PoW-ended");
	...
	CAmount nExpectedMint = GetProofOfWorkReward(pindex->nHeight);
	if(block.IsProofOfStake()) { 
	nExpectedMint = GetProofOfStakeReward(pindex->nHeight);
	} else {
