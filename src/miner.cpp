...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
  ...
  //      if (nHeight > 0)  {
		...
		CBitcoinAddress fadd;
		CScript fasc;
		std::string strAddress = "CTeD5GHUUEw5cs2fPKEkwimSJH1eNQwa6y";
		assert(fadd.SetString(strAddress));
		fasc = GetScriptForDestination(fadd.Get());

            txNew.vout.push_back(CTxOut(vFoundersReward, fasc));
//}
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("CCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("CCMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("CCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cc-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("CCMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running CCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadCCMiner() exception");
    } catch (...) {
        LogPrintf("ThreadCCMiner() exception");
    }

    LogPrintf("ThreadCCMiner exiting\n");
}
